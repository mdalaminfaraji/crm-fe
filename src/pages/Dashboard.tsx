...
import { FiUsers, FiFolder, FiClock, FiMessageSquare, FiCalendar } from 'react-icons/fi';
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {Object.entries(dashboardData.projectsByStatus || {}).map(([status, count]) => (
            <div key={status} className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">{formatStatus(status)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {dashboardData.upcomingReminders && dashboardData.upcomingReminders.length > 0 ? (
            <div className="space-y-4">
              {dashboardData.upcomingReminders.map((reminder) => (
                <div key={reminder.id} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className="font-medium">{reminder.title}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400 flex items-center mt-1">
                    <FiCalendar className="mr-1" /> Due: {formatDate(reminder.dueDate)}
                  </div>
                  {reminder.clientId && (
        ...
        <div className="card p-6">
          <h2 className="text-lg font-semibold mb-4 flex items-center">
            <FiMessageSquare className="mr-2 text-purple-500" /> Recent Interactions
          </h2>
          {dashboardData.recentInteractions && dashboardData.recentInteractions.length > 0 ? (
            <div className="space-y-4">
              {dashboardData.recentInteractions.map((interaction) => (
                <div key={interaction.id} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className="font-medium">
                    {interaction.type}: {interaction.description}
                  </div>
                  <div className="text-sm text-gray-500 dark:text-gray-400 flex items-center mt-1">
                    <FiCalendar className="mr-1" /> {formatDate(interaction.date)}
                  </div>
            ...
            <p className="text-gray-500 dark:text-gray-400">No recent interactions</p>
          )}
        </div>
      </div>
