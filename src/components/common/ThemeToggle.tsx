...
const ThemeToggle: React.FC<ThemeToggleProps> = ({ className = "" }) => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`p-2.5 rounded-full relative overflow-hidden group ${className}`}
      style={{
        backgroundColor:
          theme === "dark" ? "var(--bg-secondary)" : "var(--bg-secondary)",
    ...
    >
      ...
      <div className="relative transition-all duration-500 ease-in-out transform">
        {theme === "light" ? (
          <FiMoon className="w-5 h-5" style={{ color: "#6366f1" }} />
        ) : (
          <FiSun className="w-5 h-5" style={{ color: "#fbbf24" }} />
        )}
      </div>
